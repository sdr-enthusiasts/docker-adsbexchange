...
stats_raw = subprocess.check_output(
    ["curl", "--silent", "https://www.adsbexchange.com/myip/"]
)
...
if "ERR" in stats_json.keys():
    ...
    if stats_json[jsonkey].lower() == "not found":
        ...
        print(
            "Online statistics at: https://www.adsbexchange.com/api/feeders/?feed=%s"
            % (stats_json[jsonkey])
        )
    print("")
    datadict = dict()
    ...
    for k in datadict.keys():
        ...
        if not (beast_ok or mlat_ok):
            print(
                "No Mode-S data being received by adsbexchange, check https://adsbexchange.com/myip/"
            )
